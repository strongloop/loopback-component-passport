 ...
 */
 module.exports = function(UserIdentity) {
  ...
  // Let's create a user for that
     ...
     if(typeof options.profileToUserFields !== 'undefined')
     {
       for( var key in options.profileToUserFields )
       {

         var fieldName = options.profileToUserFields[key];

         if( fieldName.search('.') !== -1) {

           var keys = options.profileToUserFields[key];
           keys = keys.split('.');

           if(typeof profile._json[keys[0]] !== "undefined" && profile._json[keys[0]] !== "")
           {
             var value = profile._json;
           ...
           if(typeof profile._json[options.profileToUserFields[key]] !== "undefined" && profile._json[options.profileToUserFields[key]] !== "")
           {
             value = profile._json[options.profileToUserFields[key]];
             userObj[key] = value;
           }
