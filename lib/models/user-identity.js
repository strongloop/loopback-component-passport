...
function UserIdentity(UserIdentity) {
  ...
  function profileToUser(provider, profile, options) {
    ...
    if (!options.profileMapping) {
      ...
      var generatedEmail =
        (profile.username || profile.id) + '@loopback.' + (profile.provider || provider) + '.com';
  ...
  UserIdentity.login = function(provider, authScheme, profile, credentials, options, cb) {
    ...
    userIdentityModel.findOne(
      ...
      function(err, identity) {
            ...
            function(err, i) {
              ...
              return identity.user(function(err, user) {
                ...
                if (!err && user && autoLogin) {
                  return (options.createAccessToken || createAccessToken)(user, function(
                    err,
                    token
                  ) {
        ...
        // Find the user model
        var userModel =
          (userIdentityModel.relations.user && userIdentityModel.relations.user.modelTo) ||
