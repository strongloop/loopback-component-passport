...
module.exports = function(UserIdentity) {
  ...
  UserIdentity.login = function(provider, authScheme, profile, credentials,
    ...
    // KAN-597: if a profile has no id, the findOne conditions is just on the provider...
    // so the first user to match the provider was used
    if (!profile.id) {
      return cb('id is missing from the user profile');
    }
      ...
      if (identity) {
        ...
        return identity.updateAttributes({profile: profile,
          ...
          return identity.user(function(err, user) {
            // Update user with newer SSO attributes
            if (!err && user) {
              try {
                ...
                for (let attribute in userObj) {
                  if (attribute !== 'password' && !user[attribute] && userObj[attribute]) {
                    newAttributes[attribute] = userObj[attribute];
                  }
                }
