...
PassportConfigurator.prototype.buildUserLdapProfile = function(user, options) {
  ...
  for (var profileAttributeName in options.profileAttributesFromLDAP) {
    ...
    if (LdapAttributeName === 'objectGUID' || LdapAttributeName === 'objectSid') {
      if (profile[profileAttributeName].length == 14) {
        profile[profileAttributeName] = bytesToUuid(Array.prototype.slice.call(Buffer.from(profile[profileAttributeName])));
      }
    }

    // [contact schema](https://tools.ietf.org/html/draft-smarr-vcarddav-portable-contacts-00) compatibility
    if (LdapAttributeName === 'givenName') {
      if (!profile['name']) profile['name'] = {};
      profile['name'].givenName = profile[profileAttributeName];
      delete profile[profileAttributeName];
    }
    if (LdapAttributeName === 'sn') {
      if (!profile['name']) profile['name'] = {};
      profile['name'].familyName = profile[profileAttributeName];
      delete profile[profileAttributeName];
    }
    if (LdapAttributeName ==='mail') {
      profile.emails = [{value: profile[profileAttributeName]}];
      delete profile[profileAttributeName];
    }
  }
  ...
  if (!profile.id) {
    profile.id = user['uid'] || bytesToUuid(Array.prototype.slice.call(Buffer.from(profile['objectGUID'])));;
  }
  if (!profile.emails) {
