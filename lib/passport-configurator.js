...
PassportConfigurator.prototype.setupModels = function(options) {
  ...
  /**
   *  Set up property name to be used as foreignKey
   *  could use the following and avoid bringing in underscore.string but it's ugly:
   *
   *  var foreignKey=this.userModel.modelName.charAt(0).toLowerCase()+this.userModel.modelName.slice(1)+'Id';
   */
   var foreignKey=camelize(this.userModel.modelName+'Id',true);
  ...
  if (!this.userIdentityModel.relations.user) {
    // Set correct foreignKey
    this.userIdentityModel.belongsTo(this.userModel, {as: 'user', foreignKey: foreignKey});
  }

  if (!this.userCredentialModel.relations.user) {
    // Set correct foreignKey
    this.userCredentialModel.belongsTo(this.userModel, {as: 'user', foreignKey: foreignKey});
  }
};
