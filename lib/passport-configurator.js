...
PassportConfigurator.prototype.configureProvider = function(name, options) {
  ...
  var successRedirect = function(req, info){
    console.log('successredirect info', info);
    if (!!req && req.session && req.session.returnTo){
      var returnTo = req.session.returnTo;
      delete req.session.returnTo;
      return appendAccessToken(returnTo, info);
    }
    return appendAccessToken(options.successRedirect, info) ||
      (link ? '/link/account' : '/auth/account');
  };

  var appendAccessToken = function (url, info) {
    return url + '?access-token=' + info.accessToken.id + 'user-id' + info.userId ;
  };
      ...
      passport.use(name,
        new AuthStrategy(_.defaults({
          ...
          function (req, profile, done) {
            if (link) {
              if (req.user) {
                self.userCredentialModel.link(req.user.id, name, authScheme, profile, { }, options, done);
              } else {
                done('No user is logged in');
              }
            } else {
    ...
    passport.authenticate(name, _.defaults({session: session},
      ...
      if (session) {
        req.logIn(user, function(err) {
          ...
          return res.redirect(successRedirect(req, info));
        });
      } else {
        if (info && info.accessToken) {
          if (!!options.json) {
