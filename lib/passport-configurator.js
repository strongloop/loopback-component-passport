...
PassportConfigurator.prototype.configureProvider = function(name, options) {
  ...
  switch (authType) {
    ...
    // Aurélien: for use with https://github.com/panva/node-openid-client.git
    case 'openid-connect':
      ...
      if (options.state) {
        oidcState = uuidv4();
        params.state = oidcState;
      }

      const customerIssuer = new Issuer({
        issuer: options.issuer,
        authorization_endpoint: options.authorizationURL,
        token_endpoint: options.tokenURL,
        userinfo_endpoint: options.userinfoURL,
        jwks_uri: options.jwksURL,
      });

      const client = new customerIssuer.Client({
        client_id: options.clientID,
        client_secret: options.clientSecret,
        token_endpoint_auth_method: options.tokenAuthMethod || 'client_secret_basic',
        id_token_signed_response_alg: options.tokenSignAlg || 'RS256',
      });
      ...
      strategy = new AuthStrategy({
        ...
        function (req, tokenset, userinfo, done) {
          // claims can now be a function
          // https://github.com/panva/node-openid-client/releases/tag/v3.0.0
          const claims = typeof tokenset.claims === 'function' ? tokenset.claims() : tokenset.claims;
          ...
          if (options.state) {
            console.log('state', oidcState);
            if (oidcState !== tokenset.state) {
              done('ERROR: state parameter mismatch');
              return;
            }
          }
          ...
          profile.id = profile[options.externalId || 'sub']
