 ...
 * @property {Object} [additionalUserModels] Additional model classes belonging to user model
 * @property {Object} [additionalModels] Additional model classes
 * @end
 */
PassportConfigurator.prototype.setupModels = function(options) {
  ...
  if (options.additionalUserModels) {
    _.each(_.keys(options.additionalUserModels), (option) => {
      let sOption = option.split('Model');
      if (sOption.length === 2 && sOption[1] === '') {
        let modelType = sOption[0].charAt(0).toUpperCase() +
          sOption[0].substring(1).toLowerCase();
        this[option] = options.additionalUserModels[option] ||
          loopback.getModelByType(this.app.models[modelType]);
        if (!this[option].relations.user) {
          this[option].belongsTo(this.userModel, {as: 'user'});
        }
      }
    });
  }
  if (options.additionalModels) {
    _.each(_.keys(options.additionalModels), (option) => {
      let sOption = option.split('Model');
      if (sOption.length === 2 && sOption[1] === '') {
        let modelType = sOption[0].charAt(0).toUpperCase() +
          sOption[0].substring(1).toLowerCase();
        this[option] = options.additionalModels[option] ||
          loopback.getModelByType(this.app.models[modelType]);
      }
    });
  }
...
PassportConfigurator.prototype.configureProvider = function(name, options) {
  ...
  switch (authType) {
    ...
    case 'saml':
      ...
      break;
    case 'custom':
      let verifyMethod = options.verifyMethod || function() {};
