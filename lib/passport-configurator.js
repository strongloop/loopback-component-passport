    ...
    case 'local':
      strategy = new AuthStrategy(_.defaults({
        ...
        function(username, password, done) {
          ...
          console.log('passport-configurator: login local', query);
          self.userModel.findOne(query, function(err, user) {
            if (err) {
              console.log('passport-configurator: login local user find error', err);
              return done(err);
            }

            var errorMsg = 'INVALID_CREDENTIALS';
            if (user) {
              var u = user.toJSON();
              ...
              console.log('passport-configurator: login local user found', u.email);
                  ...
                  function(err, accessToken) {
                    if (err) {
                      ...
                      if(err.code === 'LOGIN_FAILED_EMAIL_NOT_VERIFIED') {
                        return done(null, false, {message: 'LOGIN_FAILED_EMAIL_NOT_VERIFIED'});
                    ...
                    if (accessToken) {
                      ...
                      console.log('passport-configurator: login success', accessToken.id);

                      done(null, userProfile, {accessToken: accessToken});
                    } else {
                      done(null, false, {message: 'LOGIN_FAILED'});
                    }
                  });
              };
  ...
  var defaultCallback = function(req, res, next) {
    ...
    passport.authenticate(name, _.defaults({session: session},
      options.authOptions), function(err, user, info) {
        console.log('passport-configurator: on authenticate start', err);
        if (err) {
          return next(err);
        }
        if (!user) {
          if (!!options.json) {
            return res.status(401).json(g.f('NOT_AUTHENTICATED'));
          }
        ...
        if (session) {
          console.log('passport-configurator: on authenticate session', session);
          ...
          console.log('passport-configurator: on authenticate info', info);
          if (info && info.accessToken) {
            if (!!options.json) {
              return res.json({
                'access_token': info.accessToken.id,
