...
PassportConfigurator.prototype.setupModels = function(options) {
  ...
  this.foreignKey = camelize(this.userModel.modelName,true) + 'Id';
  ...
  if (!this.userIdentityModel.relations.user) {
    this.userIdentityModel.belongsTo(this.userModel, {
        as: 'user',
  ...
  if (!this.userCredentialModel.relations.user) {
    this.userCredentialModel.belongsTo(this.userModel, {
        as: 'user',
        foreignKey: this.foreignKey
    });
  }
