...
PassportConfigurator.prototype.configureProvider = function (name, options) {
  ...
  function loginCallback(req, done) {
    return function (err, user, identity, token) {
     ...
     if(identity.provider === 'facebook-login' || identity.provider === 'google-login'){
      ...
      user.firstName = identity.profile.displayName.substr(0,str.indexOf(' ')) ;
      user.lastName = identity.profile.displayName.substr(str.indexOf(' ')+1) ;
      user.save(function (err) {
       if(err) {
        console.log(err);
       }
  ...
  if (link) {
    ...
    self.app.get(callbackPath, function (req, res, next) {
      var customCallback = options.customCallback || function (err, user, info) {
        ...
        req.logIn(user, function (err) {
         ...
         if (info && info.accessToken) {
          res.json({'access_token': info.accessToken.id, userId : info.userId});
         }
         else{
      ...
      passport.authenticate(name, _.defaults({session: session},options.authOptions), customCallback)(req, res, next);
    });
  }
}
