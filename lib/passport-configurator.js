...
PassportConfigurator.prototype.configureProvider = function (name, options) {
  ...
  switch (authType) {
    case 'cas':
      passport.use(name, new AuthStrategy(_.defaults({}, options),
        function (req, username, done) {
          var query = {where: {or: [
            ...
            self.userModel.findOne(query, function(err, user) {
                ...
                if (user) {
                    user.createAccessToken(1209600, function(err, accessToken) {
                        if(err) {
                            return done(err);
                        }
                        done(err, user, {'accessToken': accessToken, 'req': req});
                    });
                }
            });
      }));
