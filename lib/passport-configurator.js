...
PassportConfigurator.prototype.configureProvider = function (name, options) {
  ...
  var callbackURL = options.callbackURL || options.callbackUrl;
          ...
          if (user) {
            ...
            if (options.setAccessToken) {
              ...
              case 'username':
                ...
                break;
              default:
                throw 'No username detected';
              }

              function login(creds) {
                self.userModel.login(creds, function (err, accessToken) {
                    if (err) {
                      return done(err);
                    }
                    if (accessToken) {
   ...
   */
  if (authType === 'local') {
    self.app.post(authPath, passport.authenticate(name,
      options.fn || _.defaults({
   ...
   */
  if (link) {
    self.app[ callbackHTTPMethod ](callbackPath,
