...
PassportConfigurator.prototype.configureProvider = function (name, options) {
  ...
  if (authType === 'local') {
    ...
    self.app.get(authPath, passport.authorize(name, _.defaults({scope: scope, session: session}, options)));
  } else {
    self.app.get(authPath, passport.authenticate(name, _.defaults({scope: scope, session: session}, options)));
  }
