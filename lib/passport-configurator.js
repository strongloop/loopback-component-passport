...
PassportConfigurator.prototype.configureProvider = function (name, options) {
  ...
  switch (authType) {
    case 'cas':
      passport.use(name, new AuthStrategy(options,
        function (req, identifier, profile, options, credentials, done) {
            if(link) {
                if (req.user) {
                    self.userCredentialModel.link(req.user.id, name, authScheme, profile, credentials, options, done);
                } else {
                    done('No user is logged in');
                }
            } else {
                self.userIdentityModel.login(name, authScheme, profile, credentials, options, loginCallback(req, done));
            }
      }));
      break;
    case 'local':
