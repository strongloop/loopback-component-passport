...
PassportConfigurator.prototype.configureProvider = function(name, options) {
  ...
  var successRedirect = function(req){
    if (!!req && req.session && req.session.returnTo){
      ...
      return appendAccessToken(returnTo, req.accessToken.id);
    }
    return appendAccessToken(options.successRedirect, req.accessToken.id) ||
      (link ? '/link/account' : '/auth/account');
  };

  var appendAccessToken = function (url, token) {
