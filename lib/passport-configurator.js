 ...
 * @param {Object} cookieConfig Configuration for cookies
 * @returns {PassportConfigurator}
 */
function PassportConfigurator(app, cookieConfig) {
      ...
      if (session) {
        req.logIn(user, function(err) {
          ...
          if (info && info.accessToken) {
            if (!!options.json) {
              ...
              res.cookie(accessCookieName, info.accessToken.id,
                this.createCookieObject(accessCookieName, req, info, options));

              res.cookie(idCookieName, info.accessToken.id,
                this.createCookieObject(idCookieName, req, info, options));
            }
          }
        ...
        if (info && info.accessToken) {
          if (!!options.json) {
            ...
            res.cookie(idCookieName, user.id.toString(),
              this.createCookieObject(idCookieName, req, info, options));
          }
        }
...
PassportConfigurator.prototype.createCookieObject = function(cookieName, req, info, options) {
  var cookie = {
  ...
  if (this.cookieConfig && cookieName in this.cookieConfig) {
    var config = this.cookieConfig[cookieName];
