...
PassportConfigurator.prototype.configureProvider = function(name, options) {
  ...
  const optionsFromRequest = options.optionsFromRequest || function(req) {};
    ...
    case 'oauth 1.0':
      ...
      function(req, token, tokenSecret, profile, done) {
        if (link) {
          ...
          const OptionsForCreation = _.defaults({}, options, optionsFromRequest(req));
          self.userIdentityModel.login(provider, authScheme, profile,
            {
              token: token,
    ...
    case 'openid':
      ...
      function(req, identifier, profile, done) {
        if (link) {
          ...
          const OptionsForCreation = _.defaults({}, options, optionsFromRequest(req));
          self.userIdentityModel.login(provider, authScheme, profile,
            {identifier: identifier}, OptionsForCreation, loginCallback(req, done));
        }
      }
      );
      break;
