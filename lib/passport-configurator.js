 ...
 * @param {Object} options Ldap provider options, with mapping definition in options.profileAttributesFromLDAP
 * @return {Object} user profile
 * @end
 */
PassportConfigurator.prototype._buildUserLdapProfile = function(user, options) {
  var profile = {};
  for (profileAttributeName in options.profileAttributesFromLDAP) {
    ...
    case 'ldap':
      strategy = new AuthStrategy(_.defaults({
        ...
        function(req, user, done) {
          if (user) {
            var profile = self._buildUserLdapProfile(user, options);
...
<<<<<<< 9a2851abd8a21d140e9dc02a15e24a11fc3dc18f
          }
