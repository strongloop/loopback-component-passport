...
describe('UserIdentity', function() {
  ...
  it('supports 3rd party login with custom field mapping', function(done) {
    UserIdentity.login(
      ...
      'oAuth 2.0',
      {
        emails: [{value: 'johndoe@foobar.com'}],
        id: 'f456',
        name: {
          givenName: 'John',
          familyName: 'Doe',
        ...
        UserIdentity.create(
          ...
          function(err, identity) {
            ...
            UserIdentity.login(
              ...
              'oAuth 2.0',
              {
                emails: [{value: 'abc1@facebook.com'}],
                id: 'f456',
                username: 'xyz',
              },
  ...
  it('supports ldap login', function(done) {
    var identity = {
      emails: [{value: 'fooldap@bar.com'}],
      id: 'f123ldap',
      username: 'xyzldap',
    };
    var credentials = {accessToken: 'atldap1', refreshToken: 'rtldap1'};
    var options = {autoLogin: false};
    UserIdentity.login('ldap', 'ldap', identity, credentials, options, function(
      err,
      user,
      identity,
      token
  ...
  it('supports accessToken option before email verification', function(done) {
    User.create(
      ...
      function(err, user) {
        ...
        User.login({username: user.username, password: 'notpass'}, function(err, user) {
          assert(err, 'LOGIN_FAILED');
        });
        User.login({username: user.username, password: 'pass'}, function(err, user) {
          assert(err, 'LOGIN_FAILED');
        });
        done();
      }
